...
class Config:
    def __init__(self):
        ...
        Attributes:
            ...
            paths_to_add (list): List of paths to add to sys.path
            lumapi: Lumerical API module (if successfully imported)
            nn: nextnanopy module (if successfully imported)
            InGaAsP_models: InGaAsP models module (if successfully imported)
        """
        ...
        self.paths_to_add = self.config.get('paths_to_add', [])
        for path in self.paths_to_add:
            sys.path.insert(1, str(Path(path)))
            print(f"Added {path} to sys.path")

        self.lumapi = None
        self.nn = None
        self.InGaAsP_models = None
        self._lumapi_imported = False
        self._nn_imported = False
        self._ingaasp_imported = False
    ...
    def get_ingaasp_models(self):
        if self._ingaasp_imported:
            return self.InGaAsP_models

        try:
            import InGaAsP_models
            self.InGaAsP_models = InGaAsP_models
            print("Successfully imported InGaAsP_models")
        except ImportError as e:
            print(f"Failed to import InGaAsP_models: {e}")
            self.InGaAsP_models = None

        self._ingaasp_imported = True
        return self.InGaAsP_models
