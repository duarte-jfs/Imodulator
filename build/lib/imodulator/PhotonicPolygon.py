...
class SemiconductorPolygon:
    ...
    Attributes:
        ...
        rf_eps (float | complex | int | np.ndarray): The relative permittivity of the polygon. Material information for RF simulations if charge transport simulation. Internally, it will be converted to a callable function of the type f(omega). If you input a float, then it is constant for all frequencies, and if you give it a ``np.array`` of shape `(2,N)` then it will interpolate to give a callable of the same signature.
    ...
    rf_eps: Union[float, complex, int, np.ndarray] = 1
    ...
    _rf_eps: Union[float, complex, int, np.ndarray, Callable] = field(default=1, repr=False, init=False)

    def __post_init__(self):
        self.rf_eps = self.rf_eps  # Ensures _rf_eps is always a callable
    ...
    @property
    def rf_eps(self) -> Callable[[float], float]:
        return self._rf_eps

    @rf_eps.setter
    def rf_eps(self, value: Union[float, complex, int, np.ndarray, Callable]):
        if isinstance(value, (float, complex, int)):
            self._rf_eps = lambda omega: value
        elif isinstance(value, np.ndarray):
            self._rf_eps = lambda omega: interp1d(2 * np.pi * value[0], value[1], kind='cubic')(omega)
        elif callable(value):
            self._rf_eps = value
        else:
...
class MetalPolygon:
    ...
    def __post_init__(self):
        ...
        # Ensure rf_eps is set correctly
        self.rf_eps = self.rf_eps  # Ensures _rf_eps is always a callable
...
class InsulatorPolygon:
    ...
    Efield: Callable = lambda x, y: None
